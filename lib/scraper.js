...
function fetch(char, index = 1, waitMillisec = 0) {
  ...
  return co(function *() {
    ...
    try {
      ...
      throw new Error("Failed to fetch nicodic page");
    }
...
function scrapeAll(progressCb, charIndex = 0, pageIndex = 0, concurrency = 1, concurrencyForEachCharacter = 1, waitMillisec = 0) {
  ...
  return Promise.all(
    Array.from(katakana.slice(charIndex)).map(k => t(
