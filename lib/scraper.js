...
function scrapeCharacter(char, pageIndex = 0, progressCb, concurrency = 1, waitMillisec = 0) {
  ...
  return co(function *() {
    ...
    const result1 = yield scrapePage(char, pageIndex * 50 + 1, waitMillisec);
    ...
    return Promise.all([
      ...
      ...Array.from(new Array(pageCount - 1)).map((n, i) => {
        return t(() => scrapePage(char, (i + pageIndex + 1) * 50 + 1, waitMillisec).then(r => {
...
function scrapeAll({
  ...
  return Promise.all(
    Array.from(katakana.slice(firstCharIndex === -1 ? 0 : firstCharIndex)).map(k => t(
      () => scrapeCharacter(k, firstPageIndex, isCbEnabled ? cb : null, concurrency, waitMillisec)
    ))
  );
